...
def send_mail(despesa, current_user, motivo=None):
    ...
    departamento = (
        "administrativo"
        if despesa["departamento"] == "estoque"
        else despesa["departamento"]
    )
    ...
    if despesa["status"] == "1":
        ...
        subject = (
            "[WebSD] Uma nova solicitação foi aprovada pelo Responsável do Departamento"
        )
        ...
        recipients += [despesa["criado_por"] + "@" + MAIL_DOMAIN]
        subject = (
            "[WebSD] Sua solicitação de despesa NÃO foi aprovada pelo Depto Financeiro"
        )
        template = "status6"
    ...
    msg.body = render_template(
        "mail/" + template + ".txt", despesa=despesa, motivo=motivo
    )
    msg.html = render_template(
        "mail/" + template + ".html", despesa=despesa, motivo=motivo
    )
